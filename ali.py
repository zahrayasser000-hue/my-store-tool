...
def build_landing_page_html(data, colors):
    ...
    for badge in data.get('trust_badges', []):
        badges_html += f'<span style="background:{p};color:white;padding:8px 18px;border-radius:50px;font-size:0.85rem;font-weight:600">{badge}</span> '

    problems_html = ""
    for pt in data.get('problem_points', []):
        problems_html += f'<li style="font-size:1rem;color:#475569;line-height:1.8;margin-bottom:15px">\u274c {pt}</li>\n'

    features_html = ""
    for feat in data.get('features', [])[:4]:
        feat_img = get_ai_image(feat.get('image_search', 'feature'), 300, 300, 'product')
        features_html += f'<div style="text-align:center;padding:20px"><img src="{feat_img}" style="width:80px;height:80px;border-radius:50%;object-fit:cover;margin-bottom:10px" alt=""><h4 style="color:{p};margin-bottom:5px">{feat.get("title","")}</h4><p style="font-size:0.9rem;color:#64748b">{feat.get("desc","")}</p></div>'

    ingredients_html = ""
    for ing in data.get('ingredients', [])[:3]:
        ing_img = get_ai_image(ing.get('image_search', 'natural ingredient'), 250, 250, 'product')
        ingredients_html += f'<div style="text-align:center;padding:15px"><img src="{ing_img}" style="width:100px;height:100px;border-radius:50%;object-fit:cover;margin-bottom:10px" alt=""><h4 style="color:{p}">{ing.get("name","")}</h4><p style="font-size:0.85rem;color:#64748b">{ing.get("benefit","")}</p></div>'

    steps_html = ""
    for i, step in enumerate(data.get('how_to_use', [])[:3], 1):
        steps_html += f'<div style="text-align:center;padding:15px"><div style="width:50px;height:50px;background:linear-gradient(135deg,{g1},{g2});color:white;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 10px;font-weight:900;font-size:1.3rem">{i}</div><p style="color:#475569">{step}</p></div>'

    stats_html = ""
    for stat in data.get('stats', [])[:3]:
        stats_html += f'<div style="text-align:center;padding:20px"><div style="font-size:2.5rem;font-weight:900;color:{a}">{stat.get("number","")}</div><p style="color:#64748b;font-size:0.9rem">{stat.get("label","")}</p></div>'
    ...
    for rev in data.get('reviews', [])[:3]:
        ...
        reviews_html += f'<div style="background:white;border-radius:15px;padding:25px;box-shadow:0 4px 15px rgba(0,0,0,0.08);margin-bottom:15px"><div style="display:flex;align-items:center;gap:15px;margin-bottom:10px"><img src="{rev_img}" style="width:50px;height:50px;border-radius:50%;object-fit:cover" alt=""><div><strong>{rev.get("name","")}</strong><br><span style="font-size:0.9rem">{stars}</span></div></div><p style="color:#475569;font-style:italic">"{rev.get("comment","")}"</p><p style="color:{p};font-size:0.8rem;margin-top:5px">\u2705 مشتري موثق</p></div>'
...
<!-- Hero Section -->
<section class="section" style="background:linear-gradient(135deg,{g1},{g2});color:white;padding:60px 20px;text-align:center">
<div class="container">
...
<div>
<p style="font-size:1rem;color:#475569;line-height:1.8;margin-bottom:15px">{data.get('problem_description','')}</p>
<ul style="list-style:none;padding:0">{problems_html}</ul>
</div>
<img src="{prob_img}" style="border-radius:15px;box-shadow:0 10px 30px rgba(0,0,0,0.1)" alt="">
</div>
</div>
</section>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:30px;align-items:center">
<img src="{sol_img}" style="border-radius:15px;box-shadow:0 10px 30px rgba(0,0,0,0.1)" alt="">
<p style="font-size:1rem;color:#475569;line-height:1.8">{data.get('solution_description','')}</p>
</div>
</div>
</section>
...
<div><img src="{before_img}" style="border-radius:15px;width:100%" alt=""><p style="margin-top:10px;font-weight:700;color:#ef4444">قبل</p></div>
<div style="font-size:2rem">\u27a1</div>
<div><img src="{after_img}" style="border-radius:15px;width:100%" alt=""><p style="margin-top:10px;font-weight:700;color:{p}">بعد</p></div>
</div>
</div>
</section>
...
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px">{ingredients_html}</div>
</div>
</section>
...
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px">{steps_html}</div>
</div>
</section>
...
<div class="container">
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px">{stats_html}</div>
</div>
</section>
...
<div class="container">
<h2 style="font-size:2rem;font-weight:900;margin-bottom:20px">احصل عليه الآن!</h2>
<p style="text-decoration:line-through;opacity:0.7;font-size:1.2rem">{pricing.get('original','')} {pricing.get('currency','')}</p>
<p style="font-size:2.5rem;font-weight:900">{pricing.get('discounted','')} {pricing.get('currency','')}</p>
<p style="background:{a};display:inline-block;padding:5px 15px;border-radius:20px;font-weight:700;margin:10px 0">خصم {pricing.get('discount_percent','')}</p>
<p style="margin:15px 0;font-size:1.1rem">{data.get('urgency_text','')}</p>
<a href="#" class="btn" style="background:white;color:{p}">{cta}</a>
</div>
</section>
...
<div class="container">
<div style="font-size:4rem">\U0001f6e1</div>
<h3 style="color:{p};margin:15px 0">{data.get('guarantee_title','')}</h3>
<p style="color:#475569">{data.get('guarantee_text','')}</p>
</div>
</section>

<!-- Final CTA -->
<section class="section" style="background:linear-gradient(135deg,{a},{a}dd);text-align:center;color:white">
<div class="container">
<h2 style="font-size:2rem;font-weight:900;margin-bottom:20px">لا تفوت هذا العرض!</h2>
<a href="#order" class="btn" style="background:white;color:{a}">{cta}</a>
</div>
</section>

<!-- Footer -->
<footer style="background:#0f172a;color:white;text-align:center;padding:25px">
<p>{data.get('footer_text','')}</p>
</footer>
</body>
</html>'''
    return html
