...
st.markdown(
    '<div class="main-header"><h1>ALI Growth Engine Pro 🚀</h1>'
    '<p>منصة العمليات التسويقية | 15 قسم | +30 صورة AI</p></div>',
    unsafe_allow_html=True
)
...
def generate_lp_json(api_key, product, category):
    ...
    prompt = f"""أنت خبير تسويق رقمي ومتخصص في إنشاء صفحات هبوط عربية عالية التحويل.
المنتج: "{product}"
الفئة: "{category}"
...
{{
  ...
  "how_to_use": ["الخطوة الأولى بالتفصيل", "الخطوة الثانية بالتفصيل", "الخطوة الثالثة بالتفصيل", "الخطوة الرابعة بالتفصيل"],
  "how_to_use_images": [
...
def generate_deep_research(api_key, product, category):
    ...
    r = model.generate_content(
        ...
        f'أخرج تقريراً شاملاً بالعربية Markdown:\n'
        f'1. Avatar Sheet\n2. بحث السوق والمنافسين\n3. Offer Brief\n'
        f'4. المعتقدات الضرورية\n5. زوايا البيع PAS+FAB',
...
def extract_image_slots(data):
    ...
    for i, feat in enumerate(data.get('features', [])[:4], 1):
        add(f"IMG_FEAT_{i}", "features", feat.get('image_search', 'feature'), "feature",
            context=f"{feat.get('title', '')} {feat.get('desc', '')}")
...
def build_lp_html(data, colors, image_map=None):
    ...
    stats_html = ''.join(
        f'<div class="stat-box"><div class="sn">{st_item.get("number", "")}</div>'
        f'<div class="sl">{st_item.get("label", "")}</div></div>'
    ...
    for i, doc in enumerate(data.get('doctors', [])[:2], 1):
        ...
        docs_html += (
            ...
            f'<img loading="lazy" src="{di}" alt="{doc.get("name", "")}">'
            f'<div class="doc-info">'
            f'<div class="doc-name">{doc.get("name", "")}</div>'
            f'<div class="doc-title">{doc.get("title", "")}</div>'
            f'<div class="doc-quote"><span class="qm">"</span>{doc.get("quote", "")}<span class="qm">"</span></div>'
            f'</div></div>'
        )

    feats_html = ''
    for i, feat in enumerate(data.get('features', [])[:4], 1):
        fi = img(f"IMG_FEAT_{i}", feat.get('image_search', 'feature'), 300, 300, "feature",
                 context=f"{feat.get('title', '')} {feat.get('desc', '')}")
        feats_html += (
            f'<div class="feat-card">'
            f'<div class="feat-circle"><img loading="lazy" src="{fi}" alt="{feat.get("title", "")}"></div>'
            f'<h4>{feat.get("title", "")}</h4>'
            f'<p>{feat.get("desc", "")}</p>'
            f'</div>'
        )
    ...
    for i, ing in enumerate(data.get('ingredients', [])[:4], 1):
        ...
        ings_html += (
            ...
            f'<img loading="lazy" src="{ii}" alt="{ing.get("name", "")}">'
            f'<h4>{ing.get("name", "")}</h4>'
            f'<p>{ing.get("benefit", "")}</p>'
            f'</div>'
        )
    ...
    for i, step in enumerate(data.get('how_to_use', [])[:4], 1):
        ...
        steps_html += (
            ...
            f'<img loading="lazy" src="{si}" alt="خطوة {i}">'
            f'<div class="step-num">{i}</div>'
            f'<p class="step-txt">{step}</p>'
            f'</div>'
        )
    ...
    for i, rev in enumerate(data.get('reviews', [])[:3], 1):
        ...
        revs_html += (
            ...
            f'<div class="rev-top">'
            f'<img loading="lazy" src="{ri}" class="rev-av" '
            f'onerror="this.src=\'https://ui-avatars.com/api/?name=\'+this.alt+\'&background=random&size=250&bold=true\'">'
            f'<div><strong>{rev.get("name", "")}</strong><div class="stars">{stars}</div></div>'
            f'</div>'
            f'<p class="rev-txt">"{rev.get("comment", "")}"</p>'
            f'<span class="vbadge">✅ مشتري موثق</span>'
            f'</div>'
        )

    faq_html = ''.join(
        f'<details class="faq-item"><summary>▸ {f.get("q", "")}</summary><p>{f.get("a", "")}</p></details>'
...
<div class="sec">
  <div class="cnt">
    <h2 class="sec-title">⭐ المميزات — لماذا هو مختلف؟</h2>
...
def get_youcan_html(html):
    ...
    for m in re.finditer(r'(@media[^{]+\{)(.*?)\}\s*\}', style_content, re.DOTALL):
        ...
        for rm in re.finditer(r'([^{]+)\{([^}]+)\}', m.group(2)):
            media_inner += f'.ali-lp {rm.group(1).strip()}{{{rm.group(2).strip()}}}\n'
        media_query = m.group(1).strip()
        scoped += f'{media_query}{{{media_inner}}}\n'
    ...
    result = (
        f'<style>\n{scoped}</style>\n'
        f'<div class="ali-lp" style="direction:rtl;font-family:\'Cairo\',sans-serif;max-width:680px;margin:0 auto;">\n'
        f'{body.strip()}\n</div>'
    )
    return re.sub(r'\n\s*\n\s*\n', '\n\n', result).strip()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("📊 احسب"):
        ...
        if profit > 0:
            st.success(f"✅ مربحة! ربح {profit:.2f} مقابل إنفاق {budget:.0f}")
        else:
            st.error(f"❌ خاسرة! خسارة {abs(profit):.2f}")
